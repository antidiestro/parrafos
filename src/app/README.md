...
- Root layout (`layout.tsx`): site `metadata.title` is **Párrafos**; `description` is **Para quedar al día en un par de minutos.**; **`metadata.icons.icon`** is **`/icon.png`** (file: **`public/icon.png`**); matching **Open Graph** and **Twitter** (`summary_large_image`) tags, **`og:image` / Twitter image** at **`/og-image.png`** (file: **`public/og-image.png`**); `metadataBase` comes from **`NEXT_PUBLIC_SITE_URL`** (no trailing slash; defaults to `http://localhost:3000` if unset) so crawlers get absolute image URLs; loads **STIX Two Text** from Google Fonts (`next/font/google`) as the default UI font (Latin + Latin extended subsets). When **`NEXT_PUBLIC_GA_ID`** is set (GA4 `G-…`), **`GoogleAnalytics`** from **`@next/third-parties/google`** is included after `<body>` per the [Next.js third-party libraries guide](https://nextjs.org/docs/app/guides/third-party-libraries#google-analytics).
- Global base styles (`globals.css`): warm muted newsprint-style light background via `:root { --paper: #ebe6dc }`; the app also follows system dark mode via `@media (prefers-color-scheme: dark)` with `--paper: #0d0c0b`. `html` and `body` use `var(--paper)`, and markdown element styling is centralized in `globals.css` under `.story-md` and `.story-md-strong-open`.
