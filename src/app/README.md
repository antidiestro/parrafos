...
- Root layout (`layout.tsx`): site `metadata.title` is **Párrafos**; `description` is **Para quedar al día en un par de minutos.**; **`metadata.icons.icon`** is **`/icon.png`** (file: **`public/icon.png`**); matching **Open Graph** and **Twitter** (`summary_large_image`) tags, **`og:image` / Twitter image** at **`/og-image.png`** (file: **`public/og-image.png`**); `metadataBase` comes from **`NEXT_PUBLIC_SITE_URL`** (no trailing slash; defaults to `http://localhost:3000` if unset) so crawlers get absolute image URLs; optional **Google Tag Manager** via **`NEXT_PUBLIC_GTM_ID`** (`next/script`, `strategy="afterInteractive"`, plus `noscript` iframe) when the variable is non-empty; loads **STIX Two Text** from Google Fonts (`next/font/google`) as the default UI font (Latin + Latin extended subsets).
