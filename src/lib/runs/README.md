...
- Clustering assigns every remaining candidate source to exactly one run-scoped story cluster through a two-step process. Pass A asks the model for concrete event refs/descriptions from headline-only inputs; the prompt and schemas require at least `ceil(candidate_count / 4)` distinct events (literal counts are sent to the model). Pass B asks it to map every `source_ref` to one previously returned `event_ref` using headline + recency fields. Singleton clusters are still allowed via orphan backfill when assignments are missing/invalid. Internally, clusters still reference stable `sourceKeyFor` ids; only the LLM-facing lines use short aliases to save tokens.
- Relevance selection (`select-clusters`) only sends **multi-source** clusters (two or more articles) to the model; singleton clusters never enter that LLM call. Structured output is tiered into `primary_clusters`, `secondary_clusters`, and uncapped `diffuse_clusters`. Primary and secondary rows include `position` and `selection_reason`; only primary continues to extraction/brief generation.
