...
- **Model config:** `constants.ts` — model IDs, recency windows, and cluster-selection caps used by orchestrator and stages.

## Key Files
- `constants.ts`: Gemini model IDs, recency window hours, selection caps (`RUN_SELECT_PRIMARY_MAX` default 6, `RUN_SELECT_SECONDARY_MAX` default 4), and `parseBriefSectionComposeConstraints()` (`BRIEF_SECTION_PARAGRAPH_COUNT`, `BRIEF_SECTION_CHAR_TARGET` for the compose step). Clustering uses `RUN_CLUSTER_MODEL` (`gemini-3.1-flash-lite-preview`) in two passes: pass A discovers event refs/descriptions from headline-only lines (`c1`, `c2`, …), then pass B assigns each source alias to one discovered event using headline plus coarse recency context (`published_at`, `hours_ago` when available). Event descriptions become cluster titles.
- `console/orchestrator.ts`: wires stages and run row lifecycle.
- `console/discover-cluster-select-dry-run.ts`: reuses the same discovery, prefetch, clustering, and selection stages as the orchestrator but skips run records, novelty gate, and all persistence; logs a line per selected primary cluster plus a final summary. Tier diagnostics (primary/secondary/diffuse sizes and ids) are emitted by `select-clusters.ts`.
...
- `stages/generate-story-summaries.ts`: LLM step that emits one **structured JSON** object per story (Zod: `simpleStorySummarySchema`); `quotes` include **`speaker_context`** (role/affiliation); **`key_facts`** are longer-form detailed items. **`story_title`** is a short Spanish headline the model writes from the sources (≤200 characters); the cluster’s long `description` is only prompt context. Prompts now require explicit country naming when the story is outside Chile. `StorySummaryRow.title` matches that JSON field for compose logging, and `StorySummaryRow.sourceHeadlines` carries the deduplicated source headlines used for that story so compose can use them as angle-selection and emphasis signals. The stringified JSON is stored in `stories.markdown` / `detail_markdown` and passed to compose. Summaries run in **relevance selection order** (same order as `select-clusters` output), so published `brief_sections` and `story_articles` stay aligned with each story.
...
- Clustering assigns every remaining candidate source to exactly one run-scoped story cluster through a two-step process. Pass A asks the model for concrete event refs/descriptions from headline-only inputs; the prompt and schemas require at least `ceil(candidate_count / 4)` distinct events (literal counts are sent to the model). Pass B returns a compact `clusters` array: each item has `event_ref` plus `source_refs` as a **comma-separated** list of candidate aliases (JSON string, not a nested array—required for Gemini structured-output schema compatibility). Headline + recency fields stay in the prompt only. Singleton clusters are still allowed via orphan backfill when assignments are missing/invalid. Internally, clusters still reference stable `sourceKeyFor` ids; only the LLM-facing lines use short aliases to save tokens.
- Relevance selection (`select-clusters`) only sends **multi-source** clusters (two or more articles) to the model; singleton clusters never enter that LLM call. Structured output is tiered into `primary_clusters`, `secondary_clusters`, and uncapped `diffuse_clusters`. Primary and secondary rows include `position` and `selection_reason`; only primary continues to extraction/brief generation. The relevance prompt treats **notable passings** (deaths of widely recognized public figures with broad significance) as strong **primary** material when the cluster clearly concerns that death and coverage is multi-source.
