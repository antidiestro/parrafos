...
export async function composeBriefSections(
  ...
  const prompt = [
    ...
    "Each section is markdown for that story. Within each section, write exactly one markdown paragraph of exactly 4 sentences (compositional constraint for the LLM, not a separate storage unit).",
    "Start each section with a short inline title in bold, ending with a period, then continue in the same paragraph.",
